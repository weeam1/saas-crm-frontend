...
const InterviewedCandidates = () => {
  ...
  const handleSearch = (params) => {
    ...
    const tags = Object.entries(filteredParams).map(([key, value]) => {
      ...
      if (key === "position") {
        ...
        if (matchedOption) {
          ...
          advancedSearch.position = matchedOption._id; // ✅ Fix: Use _id, not label
        }
      }
    ...
    // Prepare query parameters - SERVER SIDE SEARCH
    const queryParams = {
      advancedSearch: JSON.stringify(advancedSearch),
      page: 1,
      limit: pageSize,
  ...
  const removeTag = (key) => {
    ...
    if (advancedSearch.position) {
      ...
      if (matchedOption) {
        advancedSearch.position = matchedOption._id; // ✅ Fix: Use _id
      }
    }
