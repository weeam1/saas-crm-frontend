	...
	const renderRoundPanel = (doc, roundKey) => (
		<Flex direction='column' gap={4}>
			...
			<Grid templateColumns={{ base: '1fr', md: 'repeat(2, 1fr)' }} gap={3}>
				{doc.evaluations?.map((item, index) => (
					<Box key={index}>
						<Box display='flex' alignItems='center' gap='2' fontWeight='500' color={colors.headingText}>
							{`${item.interviewer.fullName} Points`}
							{item.status && (
								<Icon
									as={FaClipboardList}
			...
			<DisplayField
				...
				value={`${doc.percentageScore}%`}
			/>
			<Box p={2} bg={colors.badgeInfoBg} rounded='md' display='flex' alignItems='center'>
				<Icon as={FaInfo} color={colors.badgeInfoText} mr={2} />
			...
			<FormControl>
				<FormLabel color={colors.labelColor}>Remarks</FormLabel>
				{doc?.remarks ? (
					<Box
						p={3}
						bg={colors.bgInput}
					...
					<ModalBody>
						{isLoading || isFetching ? (
							...
							<Tabs variant='unstyled'>
								...
								<TabPanels>
									<TabPanel>{renderRoundPanel(interviewDoc, 'initial')}</TabPanel>
									{interviewDoc.isMultiRound && interviewDoc.nextRound && (
										<TabPanel>{renderRoundPanel(nextRoundDoc, 'final')}</TabPanel>
									)}
								</TabPanels>
							</Tabs>
						) : (
							<Text color={colors.bodyText}>No interview data found!</Text>
						)}
					</ModalBody>
				</ModalContent>
			</Modal>
