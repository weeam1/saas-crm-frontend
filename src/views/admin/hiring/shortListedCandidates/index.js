...
const ShortListedCandidates = memo(() => {
	...
	return (
		<Box fontFamily="'DM Sans', sans-serif" bg={colors.bgDeep}  minH='100vh' p={4}>
			{!isManager && (
				<MeetingSection
					invitedCandidates={invitedCandidates}
					refetch={invitedRefetch}
					...
					{!isManager &&
						subTabsData.map((tab, index) => (
							<Button
								key={index}
								onClick={() => handleSubTabChange(index)}
								bg={activeSubTabIndex === index ? colors.goldLight : colors.bgInput}
								color={activeSubTabIndex === index ? colors.headerText : colors.bodyText}
								borderTop={
									activeSubTabIndex === index
								...
								_focus={{ outline: 'none' }}
								_hover={{
									bg: activeSubTabIndex === index ? colors.goldLight : colors.bgInputHover,
									color: activeSubTabIndex === index ? colors.headerText : colors.headingText,
								}}
								rounded='none'
								shadow='sm'
								fontSize={tabFontSize}
