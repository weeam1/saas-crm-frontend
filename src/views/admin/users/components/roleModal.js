...
const RoleModal = (props) => {
	...
	const handleCheckboxChange = (event, value) => {
		setSelectedValue(event.target.checked ? value : null);
	};

	const uniqueValues = [...new Set([selectedValue])];
	...
	// 		setSelectedValue((prevSelectedValues) => [...prevSelectedValues, item])
	// 	);
	// }, [interestRoles]);

	useEffect(() => {
		if (interestRoles?.length > 0) {
			setSelectedValue(interestRoles[0]);
		}
	}, [interestRoles]);
															...
															data = (
																<Flex align='center'>
																	<Checkbox
																		...
																		value={cell?.value}
																		isChecked={selectedValue === cell?.value}
																		onChange={(event) =>
