...
const LeadCard = ({
  ...
  agentId,
  approved,
  approvedDate: initialApprovedDate,
  rejectedDate: initialRejectedDate,
  createdDate,
  tab = "All",
  approveChangeHandler,
  _id,
  ...
  const [localApprovalStatus, setLocalApprovalStatus] = useState(initialApprovalStatus);
  const [localApprovedDate, setLocalApprovedDate] = useState(initialApprovedDate);
  const [localRejectedDate, setLocalRejectedDate] = useState(initialRejectedDate);
  const [isAcceptLoading, setIsAcceptLoading] = useState(false);
  const [isRejectLoading, setIsRejectLoading] = useState(false);
  const [isCycleModalOpen, setIsCycleModalOpen] = useState(false);
  const [leadsModal, setLeadsModal] = useState({ isOpen: false, lid: null });
  ...
  const formattedRejectedDate = safeFormattedDate(localRejectedDate);

  useEffect(() => {
    setLocalApprovalStatus(initialApprovalStatus);
    setLocalApprovedDate(initialApprovedDate);
    setLocalRejectedDate(initialRejectedDate);
  }, [initialApprovalStatus, initialApprovedDate, initialRejectedDate]);
  ...
  const { borderColor, buttonBg, buttonHoverBg, buttonColor } = getStatusStyles(localApprovalStatus);

  const handleApprovalChange = async (action) => {
    const currentDate = new Date().toISOString(); // Use current date for immediate feedback
    if (action === "accept") {
      setIsAcceptLoading(true);
      setLocalApprovedDate(currentDate); // Set local approved date immediately
    } else {
      setIsRejectLoading(true);
      setLocalRejectedDate(currentDate); // Set local rejected date immediately
    }
    ...
    try {
      ...
      if (action === "accept") setLocalApprovedDate(initialApprovedDate);
      else setLocalRejectedDate(initialRejectedDate);
    } finally {
      setIsAcceptLoading(false);
      setIsRejectLoading(false);
    }
      ...
      if (statusLower === "accepted") {
        return (
          <VStack w="100%" spacing={1} align="start">
            ...
            <Box bg="#4BFF79" w="100%" p={1} borderRadius="5px" textAlign="center">
              <Text fontSize="xs" color="black" fontWeight="bold" fontFamily="DM Sans">
                Accepted
              </Text>
            </Box>
          </VStack>
        ...
        return (
          <VStack w="100%" spacing={1} align="start">
            ...
            <Box bg="#FF4B4B" w="100%" p={1} borderRadius="5px" textAlign="center">
              <Text fontSize="xs" color="white" fontWeight="bold" fontFamily="DM Sans">
                Rejected
              </Text>
            </Box>
          </VStack>
  ...
  return (
    ...
    >
      ...
      <HStack align="start" spacing={2} w="100%" h="calc(100% - 30px)" flex="2">
        <VStack align="start" spacing={2} flex="2" w="60%" minW={0}>
          <Text fontSize="12px" fontWeight="bold" fontFamily="DM Sans" isTruncated>
            {leadName || "N/A"}
          </Text>
          <HStack spacing={4} w="100%" alignItems="flex-start">
            <VStack align="start" spacing={1} flex="1" minW={0}>
              <Text fontSize="10px" color="#BEBEBE" fontFamily="DM Sans" lineHeight="1.2">
                Source Content
              </Text>
              <Text fontSize="10px" color="#FFBB00" fontFamily="DM Sans" isTruncated lineHeight="1.2">
                {sourceContent || "N/A"}
              </Text>
            </VStack>
            <VStack align="start" spacing={1} flex="1" minW={0}>
              <Text fontSize="10px" color="#FFBB00" fontFamily="DM Sans" lineHeight="1.2">
                Time To Call
              </Text>
              <Text fontSize="10px" color="#36BE05" fontFamily="DM Sans" isTruncated lineHeight="1.2">
                {timeToCall || "N/A"}
              </Text>
            </VStack>
          </HStack>
          <HStack spacing={2} w="100%">
            <InputPair label="M Status" value={mStatus} bg="#E5B668" width="85px" color="white" />
            <InputPair
              label="Status"
          ...
          <VStack align="start" spacing={0} height="3rem" w="100%">
            ...
            <Text fontSize="xs" color="gray.500" fontFamily="DM Sans" isTruncated>
              {agentName || "N/A"}
            </Text>
          </VStack>
          <Box flex="1" w="100%" display="flex" alignItems="flex-end">
        ...
        <VStack align="start" spacing={2} flex="1" w="40%" minW={0} h="100%" justify="space-between">
          <VStack spacing={2} w="100%" align="start" pl={2}>
            <InfoPair label="City" value={<Text fontWeight="bold">{city || "N/A"}</Text>} />
            <InfoPair label="Country" value={<Text fontWeight="bold">{nationality || "N/A"}</Text>} />
          </VStack>
          <VStack align="start" spacing={1} w="100%" pl={2}>
            <Text fontSize="xs" color="#AEBAC9" fontWeight="bold" fontFamily="DM Sans">
              Info
            </Text>
            {[
              { label: "Budget", value: "N/A" },
              ...
              <HStack key={item.label} w="100%" justify="space-between" spacing={0} lineHeight="18px">
                <Text fontSize="10px" color="black" fontWeight={500} fontFamily="DM Sans">
                  {item.label}
                </Text>
                <Tooltip label={item.value} placement="right" hasArrow>
                  <Icon as={InfoIcon} color="blue.300" boxSize={3.5} cursor="pointer" />
                </Tooltip>
              </HStack>
            ))}
          </VStack>
      ...
      <Text fontSize="10px" fontFamily="'DM Sans', sans-serif" display="flex" justifyContent="flex-end">
        <Box as="span" color="#171923">
          Lead time:{" "}
        </Box>
        <Box as="span" color="black">
...
const ContactPair = ({ label, value, color }) => (
  <VStack align="start" spacing={0} flex="1" minW={0}>
    ...
    <Text fontSize={label === "Phone" ? "12px" : "xs"} color={color} fontFamily="DM Sans" isTruncated>
      {value || "N/A"}
    </Text>
  </VStack>
);
