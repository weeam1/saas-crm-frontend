...
const EditDealModal = React.memo(
	({ isOpen, onClose, initialData, onSuccess }) => {
		...
		// Theme colors - using your Navy/Gold theme
		const bgColor = 'bg.surface';
		const headerBg = 'bg.elevated';
		const headerText = 'text.heading';
		const footerBg = 'bg.surface';
		...
		const handleFileSelect = (event) => {
			...
			const isValidSize = file.size <= 5 * 1024 * 1024;
			if (!isValidType)
				return toast.error('Only PDF, DOC, DOCX files are allowed.');
			if (!isValidSize) return toast.error('Maximum size is 5MB.');
		...
		const handleFormSubmit = (data) => {
			...
			if (data.file instanceof File && data.invoiceSent)
				...
				return toast.error('Upload invoice document!');
			handleEditDeal(formData);
		};
								...
								{invoiceSent && (
									<FormControl mt={4}>
										...
										<Box
											as='button'
											onClick={() =>
												file === null && fileInputRef.current?.click()
											}
											border='2px dashed'
