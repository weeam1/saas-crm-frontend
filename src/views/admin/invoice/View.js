...
const SingleInvoice = () => {
  ...
  useEffect(() => {
    if (invoiceData?.data) {
      const bankId = invoiceData.data.bank_account_id?._id || null;
      const devId = invoiceData.data.developer_id?._id || null;
  ...
  const totals = invoiceData?.data?.totals || {};
  const invoiceNumber = invoices.length > 0 ? invoices[0].invoice_number : "-";
  ...
  return (
    <Box bg="gray.50" p={{ base: 4, md: 6, lg: 8 }} fontFamily="DM Sans">
        ...
        >
          ...
          >
            <Box w={{ base: "full", md: "50%" }} gap={3}>
              <img src={Weam} alt="Weam Elnaggar Real Estate" />
              <Text
              ...
              <Text fontSize={{ base: "xs", md: "sm" }} fontFamily="DM Sans">
              ...
              <Box w="full" color="black">
                ...
                <Box border="1px solid #eee" p={2} mt={2}>
                  <Text
                    ...
                    textAlign="left"
                  >
                    {developerData?.data?.developer_name || "N/A"}
                  </Text>
                  <Text
                    fontSize={{ base: "xs", md: "sm" }}
                    borderBottom="1px solid #eee"
          ...
          >
            <Box w={{ base: "full", lg: "35%" }} p={4} color="black">
              ...
              <Text fontFamily="DM Sans" fontSize={{ base: "xs", md: "sm" }}>
                Account Name:{" "}
                {bankAccountData?.data?.account_holder_name || "-"}
              </Text>
              <Text fontFamily="DM Sans" fontSize={{ base: "xs", md: "sm" }}>
                Account Number: {bankAccountData?.data?.account_number || "-"}
              </Text>
              <Text fontFamily="DM Sans" fontSize={{ base: "xs", md: "sm" }}>
                IBAN: {bankAccountData?.data?.iban || "-"}
              </Text>
              <Text fontFamily="DM Sans" fontSize={{ base: "xs", md: "sm" }}>
                Swift Code: {bankAccountData?.data?.swift_code || "-"}
              </Text>
              <Text fontFamily="DM Sans" fontSize={{ base: "xs", md: "sm" }}>
                Bank: {bankAccountData?.data?.bank_name || "-"}
              </Text>
              <Text fontFamily="DM Sans" fontSize={{ base: "xs", md: "sm" }}>
      ...
      <Modal isOpen={edit} size="xl" onClose={() => setEdit(false)}>
        <ModalOverlay />
        <ModalContent>
          <ModalHeader>Edit Invoice #{invoiceNumber}</ModalHeader>
          <ModalCloseButton />
          <ModalBody>
            <Edit
              isOpen={edit}
