...
// useEffect(() => {
// 			setShouldRenderMark(shouldRender);
//  }, [shouldRender]);
...
const AttendanceMark = ({
	...
	const customButtonStyle = {
		size: 'sm',
		borderRadius: 'md',
		_hover: { shadow: 'sm', transition: 'all 0.2s ease-in-out', transform: 'scale(1.03)' },
	...
	return shouldRender && data ? (
		...
		>
			{isOffDay ? (
				<Flex align='center' justify='center' gap={2}>
					<Icon as={FaBan} color={colors.mutedText} boxSize={6} />
					<Text fontSize='md' fontWeight='semibold' color={colors.mutedText}>
						Office is closed today
					</Text>
				</Flex>
			) : (
				<>
					<Text fontWeight='medium' fontSize="md" color={colors.headingText}>
						Mark Attendance
					</Text>
