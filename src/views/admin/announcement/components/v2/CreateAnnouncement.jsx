...
export default function CreateAnnouncement() {
	...
	// Colors for priority levels (using theme)
	const priorityColors = {
		1: { bg: `rgba(212, 175, 55, 0.15)`, color: colors.accentGold, label: 'Normal' },
		2: { bg: colors.badgeWarningBg, color: colors.badgeWarningText, label: 'Important' },
		3: { bg: colors.badgeErrorBg, color: colors.badgeErrorText, label: 'Critical' },
	};
		...
		<Box maxW='1000px' m='auto' p={{ base: 2, md: 4 }}>
			...
			>
				...
				>
					...
					<Text color={colors.headerSubText} mt={2} fontSize='md'>
						Send important updates to your team members and stakeholders
					</Text>
				</Box>
				...
				<Stack spacing={6} p={8}>
					...
					<FormControl
						...
						isInvalid={touched.message && !!errors.message}
					>
						<FormLabel fontWeight='semibold' color={colors.labelColor}>Message</FormLabel>
						<Textarea
							placeholder='Write your announcement message here...'
							rows={5}
							value={formData.message}
							...
							resize='vertical'
						/>
						<FormErrorMessage color={colors.badgeErrorText}>{errors.message}</FormErrorMessage>
					</FormControl>
					...
					{hasPermission('announcement', 'all_users') && (
						<FormControl
							isInvalid={touched.recipientTarget && !!errors.recipientTarget}
						>
							<FormLabel fontWeight='semibold' color={colors.labelColor}>Send To</FormLabel>
							<RadioGroup
								value={formData.recipientTarget}
								onChange={handleRecipientChange}
							>
								<Wrap spacing={4}>
									<WrapItem>
										...
										>
											<HStack>
												<Icon as={FaUsers} color={colors.accentGold} />
									...
									<WrapItem>
										...
										>
											<HStack>
												<Icon as={FaUserTag} color={colors.accentGold} />
												<Text fontSize={{ base: 'xs', md: 'sm' }} color={colors.bodyText}>
													By Role
												</Text>
											</HStack>
									...
									<WrapItem>
										...
										>
											<HStack>
												<Icon as={FaUserTie} color={colors.accentGold} />
												<Text fontSize={{ base: 'xs', md: 'sm' }} color={colors.bodyText}>
													By Team
												</Text>
											</HStack>
							...
							<FormErrorMessage color={colors.badgeErrorText}>{errors.recipientTarget}</FormErrorMessage>
						</FormControl>
					)}
					...
					{formData.recipientTarget === 'TEAM' && (
						<FormControl>
							...
							>
								{managers.map((manager) => (
									<option key={manager._id} value={manager._id} style={{ background: colors.bg, color: colors.headingText }}>
										{manager.fullName}'s Team
									</option>
								))}
							</Select>
					...
					{recipientPreview && (
						...
						>
							...
							<HStack spacing={4}>
								...
								/>
								<VStack align='start' spacing={1}>
									<Text fontWeight='bold' fontSize={{ base: 'xs', md: 'sm' }} color={colors.headingText}>
										{recipientPreview.label}
									</Text>
									<Badge
			...
			{/* Confirmation Modal */}
			<Modal isOpen={isOpen} onClose={onClose} isCentered size='lg'>
				<ModalOverlay bg={colors.overlayBg} backdropFilter='blur(10px)' />
				<ModalContent bg={colors.bg} borderRadius='2xl' border="1px solid" borderColor={colors.borderColor}>
					<ModalHeader borderBottom='1px solid' borderColor={colors.borderColor}>
						<HStack>
							<WarningIcon color={colors.accentGold} />
							<Text color={colors.headingText}>Confirm Global Announcement</Text>
						</HStack>
					</ModalHeader>
					<ModalBody py={6}>
						<Alert
						...
						>
							...
							<AlertTitle mt={4} mb={1} fontSize='lg' color={colors.badgeWarningText}>
								This will notify all users!
							</AlertTitle>
							<AlertDescription maxWidth='sm' color={colors.bodyText}>
								Are you sure you want to send this announcement to everyone in
								the system? This action cannot be undone.
							</AlertDescription>
						</Alert>
					</ModalBody>
					<ModalFooter borderTop='1px solid' borderColor={colors.borderColor} gap={3}>
						<Button
							variant='ghost'
							onClick={onClose}
							size='lg'
