...
const CharacterCounter = ({ current, max }) => {
	...
	if (percentage > 80) color = colors.badgeWarningText;
	if (percentage > 90) color = colors.badgeErrorText;
...
const TemplatePreview = ({ header, body, footer, variables }) => {
	...
	return (
		<Box w={{ base: '100%', md: '400px' }} bg={colors.bgInput} borderRadius='lg' p={4} border='1px solid' borderColor={colors.borderColor}>
			<Box
			...
			>
				{header && (
					<Text fontWeight='bold' color={colors.headingText} fontSize='sm' mb={2}>
						{header}
					</Text>
				)}
				<Text color={colors.bodyText} fontSize='sm' whiteSpace='pre-wrap' mb={2}>
					{previewBody || 'Your message template will appear here.'}
				</Text>
				{footer && (
					<Text fontSize='xs' color={colors.mutedText} pt={2}>
						{footer}
					</Text>
				)}
...
const VariableInput = ({ index, register, removeVariable, exampleValue }) => {
	...
	return (
		<Flex align='center' mb={2}>
			<Tag mr={2} p='2' bg={colors.accentGold} color={colors.headerText} minWidth='100px' borderRadius='full'>
				<TagLabel>{`{{${index}}}`}</TagLabel>
				<TagCloseButton onClick={() => removeVariable(index)} />
			</Tag>
			<Input
...
const CreateWhatsappTemplate = () => {
	...
	return (
		<>
			...
			<Flex direction={{ base: 'column', lg: 'row' }} p={6} gap={8}>
				<Box flex={1} bg={colors.bg} p={6} borderRadius='lg' boxShadow={colors.cardShadow} border='1px solid' borderColor={colors.borderColor}>
					<Flex align='center' gap={4} mb={6}>
						<Box
							...
							justifyContent='center'
						>
							<Icon as={MdCampaign} boxSize={8} color={colors.headerText} />
						</Box>
						<Box>
							<Heading size='lg' color={colors.headingText}>
								Create WhatsApp Template
							</Heading>
					...
					<form onSubmit={handleSubmit(onSubmit)}>
						<Stack spacing={6}>
							<Flex direction={{ base: 'column', md: 'row' }} gap='8'>
								{/* Template Name */}
								<FormControl isInvalid={errors.templateName}>
									<FormLabel fontSize='sm' fontWeight='medium' color={colors.labelColor}>
										Template Name
									</FormLabel>
								...
								<FormControl maxW={{ base: 'full', md: '250px' }}>
									...
									>
										{templatesLanguages?.map((lang) => (
											<option key={lang.value} value={lang.value} style={{ background: colors.bg, color: colors.headingText }}>
												{lang.label}
											</option>
										))}
									</Select>
							...
							{variables.length > 0 && (
								<Box>
									<Text fontWeight='medium' mb={2} fontSize='sm' color={colors.headingText}>
										Variable Examples
									</Text>
									<Box bg={colors.bgInput} p={3} borderRadius='md' border='1px solid' borderColor={colors.borderColor}>
										{variables.map((varIndex) => (
											<VariableInput
												key={varIndex}
												index={varIndex}
									...
									{variables.length >= VARIABLE_LIMIT && (
										<Alert status='info' mt={2} fontSize='sm' bg={colors.badgeInfoBg} color={colors.badgeInfoText}>
				...
				<Box flexShrink={0} w={{ base: '100%', md: '400px' }}>
					...
					>
						<Heading size='sm' mb={2} color={colors.headingText}>
							Best Practices
						</Heading>
						<Text fontSize='sm' color={colors.bodyText} mb={2}>
							• Use clear, concise language
						</Text>
